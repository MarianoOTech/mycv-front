...
class MockUserService {
  createUser(user: User) {
    if (user.username === 'adsoftsito') {
      return of({
        data: {
          createUser: {
            username: 'adsoftsito',
            password: 'password123',
            email: 'adsoftsito@example.com',
          },
        },
      });
    }
  ...
  tokenAuth(credential: Credential) {
    if (credential.username === 'adsoftsito' && credential.password === 'password123') {
      return of({
        data: {
          tokenAuth: {
            token: 'mock-token',
...
describe('NewUserComponent', () => {
  ...
  it('debería crear un nuevo usuario y hacer login exitoso', () => {
    ...
    // Set up user details
    component.myPayloadUser.username = 'adsoftsito';
    component.myPayloadUser.password = 'password123';
    component.myPayloadUser.email = 'adsoftsito@example.com';
